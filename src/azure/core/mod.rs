...
use hyper::header::{ContentEncoding, ContentLanguage, ContentLength, ContentType, Date,
...
#[inline]
fn add_if_exists<H: Header + HeaderFormat + Display>(h: &hyper::header::Headers) -> String {
    let m = match h.get::<H>() {
...
pub fn canonicalize_header(h: &hyper::header::Headers) -> String {
    // println!("{:?}", h);
